...
| **字段**               | **类型** | **含义**                                             | **示例值**                    |
| ---------------------- | -------- | ---------------------------------------------------- | ----------------------------- |
| `is_gone`              | `bool`   | 怪物是否已经消失（比如死亡或逃跑）                   | `false`                       |
| `move_hits`            | `int`    | 当前行动会攻击多少次                                 | `1`                           |
| `move_base_damage`     | `int`    | 当前行动的基础伤害                                   | `5`                           |
| `half_dead`            | `bool`   | 怪物是否处于半死状态（如被削减一半血量后有特殊效果） | `false`                       |
| `move_adjusted_damage` | `int`    | 受状态调整后的伤害（-1 可能表示无伤害或未定义）      | `-1`                          |
| `max_hp`               | `int`    | 怪物的最大生命值                                     | `12`                          |
| `intent`               | `str`    | 怪物的下一步意图（攻击、防御、增益等）               | `"DEBUG"`                     |
| `move_id`              | `int`    | 当前行动的编号                                       | `1`                           |
| `name`                 | `str`    | 怪物名称                                             | `"Spike Slime (S)"`           |
| `current_hp`           | `int`    | 怪物当前生命值                                       | `1`                           |
| `block`                | `int`    | 怪物的护盾值                                         | `0`                           |
| `id`                   | `str`    | 怪物的唯一标识符                                     | `"SpikeSlime_S"`              |
| `powers`               | `list`   | 怪物拥有的增益或减益状态，如中毒、虚弱等             | `[]`（空列表表示无增益/减益） |
...
1. **Spike Slime (S)**
   - 最大生命值 12，当前生命值 1（濒死状态）
   - 没有护盾 (`block: 0`)
   - 计划进行 **1 次攻击** (`move_hits: 1`)，基础伤害为 **5**
   - `move_adjusted_damage: -1` 可能表示没有实际攻击力调整
   - `intent: "DEBUG"` 可能是调试模式的特殊意图
2. **Acid Slime (M)**
   - 最大生命值 29，当前生命值 1（濒死状态）
   - 没有护盾 (`block: 0`)
   - 计划进行 **1 次攻击** (`move_hits: 1`)，基础伤害为 **10**
   - `move_adjusted_damage: -1` 可能表示没有实际攻击力调整
   - `intent: "DEBUG"` 可能是调试模式的特殊意图