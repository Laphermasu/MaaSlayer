from src.AI_model.SlayTheSpireRL.util.tokenizers import screen_type_tokenizer, map_symbol_tokenizer, relic_tokenizer, potion_tokenizer, \
    rest_tokenizer, reward_type_tokenizer, event_id_tokenizer, card_tokenizer, card_type_tokenizer, card_rarity_tokenizer
...
def get_screen_observation(game_state):
    ...
    screen_type_token = screen_type_tokenizer.texts_to_sequences([screen_type])[0][
        0] if screen_type_tokenizer.texts_to_sequences([screen_type]) else 0
...
def handle_shop_screen(screen_state, screen_type_token):
    ...
    for card in screen_state.get("cards", [])[:max_cards]:
        card_observation = [
            float(card["cost"]),
            float(card["price"]),
            float(card_tokenizer.texts_to_sequences([card["name"]])[0][0] if card_tokenizer.texts_to_sequences(
                [card["name"]]) else 0)
        ]
    ...
    for potion in screen_state.get("potions", [])[:max_potions]:
        potion_observation = [
            float(potion["price"]),
            float(potion_tokenizer.texts_to_sequences([potion["id"]])[0][0] if potion_tokenizer.texts_to_sequences(
                [potion["id"]]) else 0)
        ]
    ...
    for relic in screen_state.get("relics", [])[:max_relics]:
        relic_observation = [
            float(relic["price"]),
            float(relic_tokenizer.texts_to_sequences([relic["name"]])[0][0] if relic_tokenizer.texts_to_sequences(
                [relic["name"]]) else 0),
...
def handle_map_screen(screen_state, screen_type_token):
    ...
    # Handle missing "symbol" key
    symbol = current_node.get("symbol", "?")
    x = float(current_node.get("x", 0))
    y = float(current_node.get("y", 0))

    # Encode current node
    sequences = map_symbol_tokenizer.texts_to_sequences([symbol])
    current_node_observation = [sequences[0][0] if sequences and sequences[0] else 0, x, y]
    ...
    for node in next_nodes[:max_next_nodes]:
        ...
        node_sequences = map_symbol_tokenizer.texts_to_sequences([node_symbol])
        node_observation = [node_sequences[0][0] if node_sequences and node_sequences[0] else 0, node_x, node_y]
    ...
    return np.array([screen_type_token, first_node_chosen, *current_node_observation, boss_available,
                     *np.array(next_nodes_observation).flatten()], dtype=np.float32)
...
def handle_hand_select_screen(screen_state):
    ...
    max_selected_cards = 1  # 限制最大选中卡数量
    ...
    # **确保 `selected_observation` 长度一致**
    while len(selected_observation) < max_selected_cards:
        selected_observation.append([0.0] * 8)

    selected_observation = np.array(selected_observation, dtype=np.float32).flatten()  # (1 * 8 = 8,)
    ...
    # **确保 `hand_observation` 长度一致**
    while len(hand_observation) < max_hand_size:
        hand_observation.append([0.0] * 8)

    hand_observation = np.array(hand_observation, dtype=np.float32).flatten()  # (5 * 8 = 40,)

    # Combine all observations
    combined_observation = np.concatenate([
        np.array([float(max_cards), can_pick_zero], dtype=np.float32),  # (2,)
        selected_observation,  # (8,)
        hand_observation  # (40,)
    ])

    assert combined_observation.shape == (50,), f"Expected shape (50,), but got {combined_observation.shape}"
...
def handle_combat_reward_screen(screen_state, screen_type_token):
    ...
    for reward in rewards[:max_rewards]:
        ...
        reward_token = reward_type_tokenizer.texts_to_sequences([reward_type])[0][
            0] if reward_type_tokenizer.texts_to_sequences([reward_type]) else 0
...
def handle_card_reward_screen(screen_state, screen_type_token):
    ...
    for card in cards[:max_cards]:  # Truncate if more than max_cards
        card_name_token = card_tokenizer.texts_to_sequences([card["name"]])[0][0] if card_tokenizer.texts_to_sequences(
            [card["name"]]) else 0
        card_type_token = card_type_tokenizer.texts_to_sequences([card["type"]])[0][
            0] if card_type_tokenizer.texts_to_sequences([card["type"]]) else 0
        card_rarity_token = card_rarity_tokenizer.texts_to_sequences([card["rarity"]])[0][
            0] if card_rarity_tokenizer.texts_to_sequences([card["rarity"]]) else 0
...
def handle_boss_reward_screen(screen_state, screen_type_token):
    ...
    for relic in relics[:max_relics]:  # Truncate if more than max_relics
        relic_name_token = relic_tokenizer.texts_to_sequences([relic["name"]])[0][
            0] if relic_tokenizer.texts_to_sequences([relic["name"]]) else 0
